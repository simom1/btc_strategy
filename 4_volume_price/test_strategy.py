...
量价结合策略 - 测试入口
用法：
  python test_strategy.py btc_15m.csv btc_5m.csv
"""
...
if __name__ == '__main__':
    if len(sys.argv) >= 3:
        print("加载真实数据...")
        df_15m = load_binance_csv(sys.argv[1])
        ...
        def make_fake(periods, freq, seed=42):
            ...
            tbr   = np.random.uniform(0.3, 0.7, periods) * vol
            return pd.DataFrame({
                'open': close, 'high': high, 'low': low, 'close': close,
                'volume': vol, 'quote_volume': vol*close,
                'trades': 1000, 'taker_buy_base': tbr
            }, index=idx)
        df_15m = make_fake(32000, '15min', seed=1)
        df_5m  = make_fake(96000, '5min',  seed=2)

    print("\n── 单次全量回测 ──")
    result = run_backtest(df_15m, df_5m)
    print("\n=== 全量回测结果 ===")
    ...
    print("\n── Walk-Forward 测试 ──")
    wf = walk_forward(df_15m, df_5m, n_splits=5)

    print("\n各 Fold 明细：")
    print(wf['wf_results'].to_string(index=False))

    from report import generate_report
    generate_report(result, wf, output='report.png', strategy_name='量价结合')
