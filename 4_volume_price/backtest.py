...
========================
周期分工：
  15m → 信号（放量突破 + OBV + VWAP + taker_buy_ratio）
  5m  → 精确入场（下一根开盘）
...
import pandas as pd
import numpy as np
from strategy import add_indicators, entry_signal, calc_stops
...
def _run_segment(
    ...
    sig_15m = entry_signal(df_15m, vol_mult, tbr_long, tbr_short)

    # forward-fill 到 5m，shift(1) 避免超前
    idx       = df_5m.index
    sig_ff    = resample_signal(sig_15m,              idx).shift(1)
    sl_ff     = resample_signal(df_15m['swing_low'],  idx).shift(1)
    sh_ff     = resample_signal(df_15m['swing_high'], idx).shift(1)
    atr_ff    = resample_signal(df_15m['atr'],        idx).shift(1)
    ...
    for i in range(20, len(df_5m) - 1):
        ...
        # ── 空仓：检查信号 ──
        direction = int(sig_ff.iloc[i])
        if direction == 0:
            continue

        next_open   = df_5m['open'].iloc[i + 1]
        swing_lo    = sl_ff.iloc[i]
        swing_hi    = sh_ff.iloc[i]
        atr_v       = atr_ff.iloc[i]

        sl, tp = calc_stops(next_open, swing_lo, swing_hi,
                            atr_v, direction, tp_mult=tp_mult)
...
def run_backtest(
    ...
    trades, _ = _run_segment(df_15m, df_5m, initial_capital,
                              vol_mult, tbr_long, tbr_short, tp_mult)
