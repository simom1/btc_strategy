...
if __name__ == '__main__':
    if len(sys.argv) >= 5:
        ...
        def make_fake(periods, freq, seed=42):
            ...
            close = 30000 + np.cumsum(np.random.randn(periods) * 100)
            high  = close + np.abs(np.random.randn(periods) * 80)
            low   = close - np.abs(np.random.randn(periods) * 80)
    ...
    generate_report(result, wf, output='report.png', strategy_name='均值回归')
